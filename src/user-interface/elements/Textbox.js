...
export default class Textbox extends Panel {
	...
	constructor(x, y, width, height, text, options = {}) {
		...
		this.shouldCenterText = options.shouldCenterText ?? false;
	...
	renderText() {
		...
		this.pageToDisplay?.forEach((line, index) => {
			context.fillText(
				line, 
				this.shouldCenterText
					? this.position.x + this.dimensions.x / 2 - 2
					: this.position.x + this.padding, 
				this.shouldCenterText
					? this.position.y + index * this.fontSize + 7 + this.padding 
