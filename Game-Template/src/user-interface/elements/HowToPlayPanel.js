import Color from "../../enums/Color.js";
import SoundName from "../../enums/SoundName.js";
// import SoundName from "../../enums/SoundName.js";
import { CANVAS_HEIGHT, CANVAS_WIDTH, context, keys, sounds, timer } from "../../globals.js";
import Panel from "./Panel.js";

export default class HowToPlayPanel extends Panel {
	static PANEL = {
		x: 0,
		y: 0,
		width: CANVAS_WIDTH, 
		height: CANVAS_HEIGHT / 32
	};

	/**
	 * A Textbox element is a Panel with text overlaid on top.
	 * The text can be "advanceable" meaning the player can hit
	 * enter or space to advance to the next "page" of text.
	 * If not advanceable, the controlling state will take care
	 * of closing the Textbox.
	 *
	 * @param {number} x
	 * @param {number} y
	 * @param {number} width
	 * @param {number} height
	 * @param {string} text
	 * @param {object} options Font size/colour/family and whether the textbox should be advanceable.
	 */
	constructor(x, y, width, height, textsArray, options = {}) {
		super(x, y, width, height, options);

		this.titleFontSize = options.titleFontSize ?? Panel.FONT_SIZE;
		this.textFontSize = options.textFontSize ?? Panel.FONT_SIZE;
		this.fontColour = options.fontColour ?? Color.Black;
		this.fontFamily = options.fontFamily ?? Panel.FONT_FAMILY;
		this.isAdvanceable = options.isAdvanceable ?? true;
		this.nextArrowAlpha = 1;
		this.endOfText = false;
		this.isClosed = false;

		this.title = textsArray[0]
		this.texts = textsArray;

		this.pages = this.createPages(textsArray, this.dimensions.x - this.padding * 3);

		if (this.isAdvanceable) {
			this.arrowTimer = this.flashAdvanceableArrow();
		}


		this.next();
	}

	update() {
		if (keys.Enter || keys[' ']) {
			keys.Enter = false;
			keys[' '] = false;

			// sounds.play(SoundName.SelectionChoice);

			this.next();
		}
	}

	render() {
		super.render();

		context.save();

		this.renderTitle();
		this.renderText();

		if (this.isAdvanceable) {
			this.renderNextArrow();
		}

		context.restore();
	}

	renderTitle(){
		context.textBaseline = 'top';
		context.font = `${this.titleFontSize}px ${this.fontFamily}`;
		context.fillStyle = this.fontColour;

		context.fillText(
			this.title, 
			this.position.x + (CANVAS_WIDTH / 2) - context.measureText(this.title).width / 2, 
			this.position.y + 20 + this.padding
		);
	}

	renderText() {
		context.textBaseline = 'top';
		context.font = `${this.textFontSize}px ${this.fontFamily}`;
		context.fillStyle = this.fontColour;

		this.pageToDisplay?.forEach((line, index) => {
			context.fillText(
				line, 
				this.position.x + this.padding * 2, 
				this.position.y + index * (this.textFontSize + 10) + this.padding * 7
			);
		});
	}

	renderNextArrow() {
		context.beginPath();
		context.translate(this.position.x + this.dimensions.x - 20, this.position.y + this.dimensions.y - 15);
		context.moveTo(0, 0);
		context.lineTo(10, 0);
		context.lineTo(5, 5);
		context.lineTo(0, 0);
		context.globalAlpha = this.nextArrowAlpha;
		context.fill();
		context.closePath();
	}

	/**
	 * @param {string} textsArray
	 * @param {number} width The width of the textbox.
	 * @returns The paginated text based on the specified width.
	 */
	createPages(textArray, width) {
		const pages = [];

		// Remove the title from the array
		textArray = textArray.slice(1, textArray.length);
		
		const lines = this.getLines(textArray, width);
		const linesPerPage = Math.floor((this.dimensions.y - 300 - this.padding) / this.textFontSize);
		const maxPages = Math.ceil(lines.length / linesPerPage);

		for (let page = 0; page < maxPages; page++) {
			pages.push([]);

			for (let line = 0; line < linesPerPage; line++) {
				if (lines.length === 0) {
					break;
				}

				pages[page][line] = lines.splice(0, 1);
			}
		}

		return pages;
	}

	next() {
		sounds.play(SoundName.HowToPlaySelect);

		if (this.endOfText) {
			this.toggle();
			this.arrowTimer.clear();
			this.isClosed = true;
		}
		else {
			this.nextPage();
		}
	}

	nextPage() {
		this.pageToDisplay = this.pages.splice(0, 1)[0];

		if (this.pages.length === 0) {
			this.endOfText = true;
		}
	}

	/**
	 * @param {string} text
	 * @param {number} maxWidth
	 * @returns The separated text lines based on the given maxWidth.
	 */
	getLines(textArray, maxWidth) {
		const wordsByLine = textArray; 
		const lines = [];

		wordsByLine.forEach((line) => {
			const words = line.replace(/\t+/g, '').split(" "); // Remove any tab characters.

			let currentLine = words[0];

			context.font = `${this.textFontSize}px ${this.fontFamily}`;

			for (let i = 1; i < words.length; i++) {
				const word = words[i];
				const width = context.measureText(currentLine + " " + word).width;

				if (width < maxWidth) {
					currentLine += " " + word;
				}
				else {
					lines.push(currentLine);
					currentLine = word;
				}
			}

			lines.push(currentLine);
		});
		return lines;
	}

	flashAdvanceableArrow() {
		const action = () => {
			this.nextArrowAlpha = this.nextArrowAlpha === 1 ? 0 : 1;
		};
		const interval = 0.75;

		return timer.addTask(action, interval);
	}
}
