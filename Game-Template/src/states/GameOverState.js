...
export default class GameOverState extends State {
	constructor(playState, options = {}) {
		...
		this.correctCountries = playState.map.correctCountries.length;
		...
		sounds.play(SoundName.GameLose);

		this.titleTextLines = this.getLines(options.titleMessage, CANVAS_WIDTH - 300);
	...
	update(dt){
		...
		if (keys.Enter) {
			...
			// Add the record 
			if (this.isPreviousRecord()){
				PreviousRecordsManager.addPreviousRecord(this.correctCountries, this.playState.timer);
			}
	...
	/**
	 * Checks if the amount of correct countries the player got is high enough to be a previous record.
	 * @returns True if a previous record; otherwise false.
	 */
	isPreviousRecord() {
		return PreviousRecordsManager.loadPreviousRecords().some((previousRecords) => this.correctCountries > previousRecords.correctCountries);
	}
